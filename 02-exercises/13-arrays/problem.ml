...
let double_selectively array indices : unit = List.iter indices ~f: (
  fun (i) -> array.(i) <- (array.(i) * 2)
)
...
let double_matrix matrix : unit = Array.iter matrix ~f: (
  fun arr -> Array.iteri arr ~f: (
    fun i x -> arr.(i) <- x * 2
  )
)
